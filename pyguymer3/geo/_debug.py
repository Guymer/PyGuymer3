...
def _debug(
    ...
    *,
    ...
    for i, badGeom in enumerate(extract_points(badGeoms, onlyValid = False)):
        ...
        with open(f"debug{prefix}Point{i:d}.csv", mode = "wt", encoding = "utf-8") as fObj:
    ...
    for i, badGeom in enumerate(extract_lines(badGeoms, onlyValid = False)):
        ...
        with open(f"debug{prefix}LineString{i:d}.csv", mode = "wt", encoding = "utf-8") as fObj:
    ...
    for i, badGeom in enumerate(extract_polys(badGeoms, repair = False, onlyValid = False)):
        ...
        with open(f"debug{prefix}Polygon{i:d}.exterior.csv", mode = "wt", encoding = "utf-8") as fObj:
        ...
        for j, interior in enumerate(badGeom.interiors):
            ...
            with open(f"debug{prefix}Polygon{i:d}.interior{j:d}.csv", mode = "wt", encoding = "utf-8") as fObj:
