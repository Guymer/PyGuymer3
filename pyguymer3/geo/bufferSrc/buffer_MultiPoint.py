def buffer_MultiPoint(multipoint, dist, kwArgCheck = None, debug = False, fill = 1.0, fillSpace = "EuclideanSpace", nang = 19, simp = 0.1, tol = 1.0e-10):
    ...
    multipoint : shapely.geometry.multipoint.MultiPoint
            the MultiPoint
    dist : float
            the Geodesic distance to buffer each point within the MultiPoint by (in metres)
    debug : bool, optional
    ...
    nang : int, optional
            the number of angles around each point within the MultiPoint that are calculated when buffering
    simp : float, optional
